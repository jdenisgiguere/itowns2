...
GeoJSONToThree.convert = function convert(geoJson) {
    const geometries = [];
    ...
    for (let f = 0; f < features.length; f++) {
        ...
        const geom = this.geomFromGeoJSON(type, coords);

        const threeGeom = this.geomToThree(geom);
    ...
    const geomIndexBuffer = new Float32Array(totalSize);
    let idx = 0;
...
GeoJSONToThree.setUpBarycentricCoordinates = function setUpBarycentricCoordinates(geometry) {
...
GeoJSONToThree.geomToThree = function geomToThree(geom) {
...
GeoJSONToThree.geomFromGeoJSON = function geomFromGeoJSON(type, coords) {
...
GeoJSONToThree.geomFromPolyhedralSurface = function geomFromPolyhedralSurface(coords) {
    let t;
    let v;
    let i;
    let j;
    ...
    const position = new Float32Array(9 * nPoints);
    ...
    for (i = 0, t = 0; t < coords.length; t++) {
        ...
        const vect1 = [positionPolygon[3] - positionPolygon[0],
            positionPolygon[4] - positionPolygon[1],
            positionPolygon[5] - positionPolygon[2],
        ];
        const vect2 = [positionPolygon[6] - positionPolygon[0],
            positionPolygon[7] - positionPolygon[1],
            positionPolygon[8] - positionPolygon[2],
        ];
        const vectProd = [vect1[1] * vect2[2] - vect1[2] * vect2[1],
        ...
        for (v = 0; v < triangles.length; v += 3) {
            const v1 = triangles[v];
            const v2 = triangles[v + 1];
            const v3 = triangles[v + 2];
            if (v1 > v2 && v1 > v3) {
                triangles[v + 2] = v1;
                if (v2 > v3) {
                    triangles[v] = v3;
